...
export default async function QuotationDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="p-8">
    ...
    <div className="invoice-detail-heading no-print"><div><p className="eyebrow">Quotation record</p><h1 className="text-3xl font-bold">{project.name}</h1><p className="muted">{project.client.name} · {project.startDate.toISOString().slice(0, 10)}</p></div><QuotationActions /></div>
    <article className="quotation-paper">
      <header className="quotation-header"><div><p className="eyebrow">Quotation</p><h2>{project.name}</h2><p>{project.description || "Quotation for creative production services."}</p></div><strong>{formatCurrency(project.quotationAmount)}</strong></header>
